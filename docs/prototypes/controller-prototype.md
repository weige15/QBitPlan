...
**Evidence.** On the constructed fixture, exact-profile hit rates are 0.39% for
the static profile, 71.29% for direct query-only scoring, 1.27% for the additive
independent baseline, and 100.00% for the causal interaction-aware planner.
These values test whether the intended code paths can express query-only and
prefix-dependent mappings; they are not estimates of QBitPlan performance.
...
only those engineered properties.
...
**Proposed Design.** Generate the deterministic prototype artifacts with:

```bash
python -m scripts.run_controller_prototype \
  --output-dir artifacts/controller-prototype \
  --seed 20260805
```
...
```bash
pytest -q tests/test_controller_prototype.py
```

**Proposed Design.** The command writes:

- `predictions.csv`: one target and four selected profiles per synthetic query,
  with per-row schema, evidence-class, and claim-boundary labels;
- `metrics.json`: evidence label, claim boundary, hit rates, per-group bit
  accuracy, training MSE, and paired bootstrap intervals;
- `controller-prototype.svg`: the report figure.
