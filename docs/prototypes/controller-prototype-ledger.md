...
| Accepted for evidence boundary | Treat the historical empty `P_exec` as an old pre-CPU-first inventory, not current state | Run `c044b28a...` is superseded for current wording; the newer smoke run completed four declared variants | A sealed current CPU-first 256-profile inventory |
| Accepted for evidence boundary | Keep the four-profile smoke result separate from the full inventory | Smoke run `f7d1e51c...` is directly measured executable-path evidence only and cannot establish full `P_exec` | A sealed complete 256-profile inventory |
| Unresolved | Which profiles are executable in the current CPU-first full inventory? | The current attempt `issue32-functional-quality-20260805-03` timed out before sealing | New UUID-pinned RTX 3090 attempt with the same controls and complete immutable inventory |
...
| CP-4 | Engineering behavior | `Stage1ControllerArtifactAdapter` maps typed profile, query-feature, target-set, and causal-prefix artifacts into the existing controller interfaces without final-data or causal fallback leakage | `qbitplan/stage1/controller_artifacts.py`, `tests/test_controller_artifact_adapter.py` | Adapter contract and validation only; not Stage-1 controller evidence |
...
| EXEC-2 | Directly measured smoke path | The newer CPU-first smoke attempt completed BF16, all-4, all-8, and mixed executable paths and recorded absolute NVML resident bytes | Local smoke bundle `f7d1e51c...` and `smoke-summary.json` | Two-query non-evidentiary executable-path and memory observations only; not full `P_exec`, quality, or generalization |
